	...
	: QMainWindow(parent)
{
	...
	connect(this->playerUiBtnAddMedia.get(), &QAction::triggered, this, [=]() {
		QStringList filenames = QFileDialog::getOpenFileNames(this, tr("Open"), fileDirLastOpened, tr("Video Files (*.wmv *.gif *.mp4 *.avi *.mp3 *.webm);;AllFiles (*)"));
...
bool UnnecessaryMediaPlayer::validCmdLineIsRead()
{
	...
	{
		QStringList validSuffixes = { "wmv", "gif", "mp4", "avi", "mp3", "webm" };
